...
const CompositeOperations_ts_1 = require("./CompositeOperations.ts");
...
const RenderableList_ts_1 = require("./RenderableList.ts");
const Scene_ts_1 = require("./Scene.ts");
const Sound_ts_1 = require("./Sound.ts");
...
exports.Engine = (() => {
    let _autoScale = true;
    let _canvas = Static_ts_1.Static.getElement("game") || Static_ts_1.Static.getElements(CANVAS_ELEMENT)[0];
    ...
    let _input = new Input_ts_1.Input();
    let _lastRender;
    let _name = "Game";
    ...
    let _scene;
    let _sound = new Sound_ts_1.Sound();
    let _transition;
    ...
    class Engine {
        ...
        static get center() {
            return new Point_ts_1.Point(this.centerX, this.centerY);
        }
        static get centerX() {
        ...
        static colorize(image, fillStyle, compositeOperation = CompositeOperations_ts_1.CompositeOperations.SOURCE_IN) {
            const input = Static_ts_1.Static.getImage(image);
            const output = ACL_ts_1.ACL.document.createElement(CANVAS_ELEMENT);
        ...
        static load(namespace) {
            const container = localStorage[namespace !== null && namespace !== void 0 ? namespace : Engine.getName()];
            let result;
            try {
        ...
        static paint(renderable, index = 0) {
            if (index >= _renderableLists.length) {
                for (let i = _renderableLists.length; i <= index; ++i) {
                    _renderableLists.push(new RenderableList_ts_1.RenderableList());
                }
            }
        ...
        static rotate(image, degrees) {
            ...
            context.rotate(Static_ts_1.Static.toRadians(degrees));
        ...
        static setFrameTime(frameTime) {
            _frameTime = frameTime !== null && frameTime !== void 0 ? frameTime : DEFAULT_FRAME_TIME;
        }
        ...
        static setName(name) {
            _name = name;
            document && (ACL_ts_1.ACL.document.title = _name);
        }
        static stopTheme() {
            _sound.stopTheme();
        }
        ...
        static image(id, isMirror, isFlip) {
            if (isFlip && isMirror) {
                ...
                _imageCache[id].flipMirror = invert(Static_ts_1.Static.getElement(id), true, true);
            ...
            if (isFlip) {
                ...
                _imageCache[id] = _imageCache[id] || {};
                _imageCache[id].flip = Engine.flip(Static_ts_1.Static.getElement(id));
                return _imageCache[id].flip;
            }
            if (isMirror) {
                ...
                _imageCache[id] = _imageCache[id] || {};
                _imageCache[id].mirror = Engine.mirror(Static_ts_1.Static.getElement(id));
                return _imageCache[id].mirror;
            }
            return Static_ts_1.Static.getElement(id);
        }
        static point(x, y) {
    ...
    function boot(canvas, context) {
        ...
        focus();
        scale();
        const images = Static_ts_1.Static.getElements("img");
        ...
        context.fillStyle = Color_ts_1.Color.Blue;
    ...
    function focus() {
        ACL_ts_1.ACL.window.focus();
    }
    function initScene() {
        if (!_scene) {
            throw new Error("Could not get the next scene");
        }
        _scene.scene = new Point_ts_1.Point();
    ...
    function loop() {
        ...
        _sound.update();
        ACL_ts_1.ACL.window.requestAnimationFrame(render);
    }
    function render() {
    ...
    function scale() {
        ...
        let width, height;
        if (_keepAspect) {
            let proportion = ACL_ts_1.ACL.window.innerWidth / _canvas.width;
            if (ACL_ts_1.ACL.window.innerHeight < _canvas.height * proportion) {
                proportion = ACL_ts_1.ACL.window.innerHeight / _canvas.height;
            }
            width = _canvas.width * proportion;
            height = _canvas.height * proportion;
        }
        else {
            width = ACL_ts_1.ACL.window.innerWidth;
            height = ACL_ts_1.ACL.window.innerHeight;
        }
        _realWidth = width;
        _realHeight = height;
